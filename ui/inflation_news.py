...
st.title("Inflation News Decomposition")
st.caption("Model 1B governed live forecast changes: new data, revisions, refit, and policy")
repository = MacroRepository()
repository.initialise()

latest = repository.query_df(
    "SELECT run_id, run_timestamp FROM inflation_live_runs ORDER BY run_timestamp DESC LIMIT 1"
)
if latest.empty:
    st.info("Run the governed inflation nowcast first.")
    st.stop()
run_id = str(latest.iloc[0]["run_id"])

if st.button("Rebuild latest inflation news decomposition"):
    with st.spinner("Re-estimating counterfactual inflation forecasts..."):
        try:
...
runs = repository.query_df(
    """
    SELECT * FROM inflation_news_runs
...
selected_target = st.selectbox("Target", runs["target_series"].tolist())
...
if row["status"] == "success":
    ...
    col3.metric("Total change", f"{row['total_change']:+.3f} pp")
    impacts = pd.DataFrame(
        [
            {"Channel": "New data", "Impact, pp": row["new_data_impact"]},
            {"Channel": "Revisions/removals", "Impact, pp": row["revision_impact"]},
            {"Channel": "Model refit", "Impact, pp": row["model_refit_impact"]},
            {"Channel": "Stable-policy change", "Impact, pp": row["policy_change_impact"]},
            {"Channel": "Residual interaction", "Impact, pp": row["residual_interaction"]},
        ]
    )
    ...
    contributions = repository.query_df(
        ...
        FROM inflation_news_contributions
    ...
    if contributions.empty:
        ...
        st.dataframe(contributions.drop(columns=["details_json"]), use_container_width=True, hide_index=True)

    changes = repository.query_df(
        """
        SELECT series_id, observation_date, change_type, previous_value,
               current_value, value_change
        FROM inflation_release_changes
        WHERE decomposition_id = ?
