...
from macropulse.labour.candidate_validation import run_labour_candidate_validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the Model 1C stable 15-decision labour policy, its prior-only "
...
def main() -> None:
    args = parse_args()
    result = run_labour_candidate_validation(backtest_id=args.backtest_id)
    print("Model 1C candidate validation complete")
    print(f"Validation ID: {result['validation_id']}")
    print(f"Backtest ID: {result['backtest_id']}")
    print(f"Calibration ID: {result['calibration_id']}")
    print(f"Prior validation ID: {result['base_validation_id']}")
