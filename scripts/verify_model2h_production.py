...
def main() -> int:
    ...
    try:
        ...
        for ancestor in (EXPECTED_2G_CLOSURE, EXPECTED_2H_CLOSURE):
            c = subprocess.run(["git","merge-base","--is-ancestor",ancestor,"HEAD"], cwd=ROOT)
            if c.returncode != 0:
                errors.append("HEAD does not descend from " + ancestor)
        delta = names(git("diff","--name-only",EXPECTED_2G_CLOSURE+".."+EXPECTED_2H_CLOSURE))
        if delta != EXPECTED_DELTA:
            extra = sorted(delta - EXPECTED_DELTA)
            missing = sorted(EXPECTED_DELTA - delta)
            if extra: errors.append("Unexpected frozen 2H paths: " + ", ".join(extra))
            if missing: errors.append("Missing frozen 2H paths: " + ", ".join(missing))
        for path in sorted(FROZEN_CONTENT):
            c = subprocess.run(["git","diff","--quiet",EXPECTED_2H_CLOSURE,"--",path], cwd=ROOT)
            if c.returncode != 0:
                errors.append("Frozen 2H content changed: " + path)
        payload = json.loads(git("show", EXPECTED_2H_CLOSURE + ":MODEL2H_RELEASE_CANDIDATE_CONTRACT.json"))
        ...
        if selected.get("candidate_id") != "a69878bf644615c5" or selected.get("lags") != 2 or abs(float(selected.get("shrinkage",-1))-0.1)>1e-12:
        ...
        for token in ("python scripts/verify_model2h_production.py","tests/test_model2h_production.py"):
