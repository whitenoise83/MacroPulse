...
from macropulse.inflation.config import get_inflation_series_definitions


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download Model 1B inflation data from FRED.")
    parser.add_argument(
        "--include-initial",
        action="store_true",
...
def main() -> None:
    ...
    total = 0
    for definition in get_inflation_series_definitions():
        ...
        try:
            ...
            latest = client.get_observations(
                definition.series_id, definition.start_date, vintage_type="latest"
            )
            ...
            if args.include_initial:
                initial = client.get_observations(
                    definition.series_id, definition.start_date, vintage_type="initial"
                )
            ...
            print(f"  ERROR: {exc}")
            raise
    print(f"Finished. Inserted or refreshed {total:,} inflation-data rows.")
