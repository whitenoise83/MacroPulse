...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate a governed Model 1B live candidate run."
    )
    parser.add_argument("--run-id", help="Governed live run ID. Defaults to the latest run.")
    args = parser.parse_args()
    result = run_inflation_operational_validation(run_id=args.run_id)
    print("Model 1B governed live validation complete")
