...
from macropulse.labour.dataset import LabourDataset, build_target_dataset
from macropulse.labour.models import LabourModelResult, fit_model_suite
...
from macropulse.labour.stages import LABOUR_STAGE_MAP, infer_live_labour_stage
...
def estimate_target_models(
...
) -> tuple[LabourDataset, dict[str, LabourModelResult]]:
    ...
    fitted = fit_model_suite(dataset.X, dataset.y, dataset.forecast_X, config)
    return dataset, {result.model_name: result for result in fitted}


def latest_candidate_evidence(repository: MacroRepository) -> CandidateEvidence:
    rows = repository.query_df(
        """
        SELECT validation_id, backtest_id, report_path, passed_gates, summary_json,
               created_at
    ...
    for row in rows.itertuples(index=False):
        ...
        if summary.get("validation_type") == "candidate_policy":
            return CandidateEvidence(
                ...
                passed_checks=int(row.passed_gates),
            )
    raise RuntimeError(
        "No passing Model 1C candidate-policy validation is stored. "
        "Run scripts\\run_labour_candidate_validation.py first."
    )
...
def live_shadow_selection(
    ...
    forecast_stage: str,
    target_period: pd.Period | str,
    components: dict[str, LabourModelResult],
...
def model_state_hash(component_rows: pd.DataFrame, coefficient_rows: pd.DataFrame) -> str:
...
def governance_signature_payload(
    run_record: dict[str, Any], forecast_rows: pd.DataFrame
) -> dict[str, Any]:
    fields = [
        "run_id", "model_id", "model_version", "candidate_validation_id",
        "backtest_id", "information_cutoff", "information_set_hash",
        "model_state_hash", "config_hash", "code_hash", "git_commit",
    ]
    forecasts = forecast_rows[
        [
            "target_series", "target_period", "forecast_stage",
...
def run_governed_labour_nowcast(
    ...
    repository.register_model_identity(
        identity.as_dict(),
        notes=f"Model 1C v{identity.model_version} {identity.lifecycle_status} live identity.",
    )
    evidence = latest_candidate_evidence(repository)
    definitions = get_labour_series_definitions()
    observations = repository.latest_observations([item.series_id for item in definitions])
    if observations.empty:
        raise RuntimeError("No labour observations are stored. Run download_labour_data.py first.")
    ...
    if observations.empty:
        raise RuntimeError("No labour observations are available at the requested information cutoff.")
    ...
    selector_config = ShadowSelectorConfig(
        ...
        switch_hurdle=float(config.get("policy_switch_hurdle", 0.03)),
        mae_guard=float(config.get("policy_mae_guard", 1.05)),
        median_guard=float(config.get("policy_median_guard", 1.10)),
        tail_guard=float(config.get("policy_tail_guard", 1.10)),
        max_error_guard=float(config.get("policy_max_error_guard", 1.15)),
        directional_guard=float(config.get("policy_directional_guard", 0.03)),
    )
    ...
    for definition in target_definitions():
        ...
        stage = infer_live_labour_stage(dataset.target_period, cutoff, release_date)
        ...
        interval = live_interval_from_backtest(
            ...
            decay=float(config.get("interval_calibration_decay", 0.94)),
        ...
        target_metrics[definition.series_id] = {
            ...
            "target_period": str(dataset.target_period),
            "forecast_stage": stage,
            "forecast_stage_label": LABOUR_STAGE_MAP[stage].label,
    ...
    base_run: dict[str, Any] = {
        ...
        "notes": (
            f"Model 1C v{identity.model_version} {identity.lifecycle_status}. The stable policy "
            "controls the headline; the adaptive selector is stored as a shadow challenger only."
        ),
    }
    ...
    legacy_forecasts = components.rename(
        ...
        [
            ...
            "display_decimals", "target_period", "model_name", "point_forecast",
            "lower_80", "upper_80", "diagnostics_json", "created_at",
        ]
    ]
    repository.save_labour_outputs(legacy_run, legacy_forecasts, coefficients)
    repository.save_labour_live_outputs(live_run, forecasts, components)
    repository.save_labour_live_information_set(run_id, observations)
    ...
    if build_news:
        ...
        try:
            news_result = build_labour_news_decomposition(repository, run_id)
        except Exception as exc:
            repository.record_labour_news_failure(run_id, str(exc))
