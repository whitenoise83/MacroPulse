...
def build_labour_news_decomposition(
    repository: MacroRepository,
    current_run_id: str,
) -> dict[str, Any]:
    current_run = repository.query_df(
        "SELECT * FROM labour_live_runs WHERE run_id = ?", [current_run_id]
    )
    if current_run.empty:
        raise RuntimeError(f"Governed labour run {current_run_id} does not exist.")
    ...
    if previous.empty:
        ...
        repository.save_labour_news_outputs(
    ...
    previous_info = _normalise_information_set(
        repository.labour_live_information_set(previous_run_id)
    )
    current_info = _normalise_information_set(
        repository.labour_live_information_set(current_run_id)
    )
