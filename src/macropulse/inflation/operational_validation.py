...
from macropulse.inflation.live import governance_signature, model_state_hash
from macropulse.inflation.policy import SELECTED_INTERVAL_METHOD, STABLE_POLICY, policy_model
...
def evaluate_live_run_frames(
    ...
    checks.append(
        _check(
            "Data integrity",
            "Exactly one governed headline exists for each inflation target",
    ...
    invalid_stages = sorted(
        ...
        .difference(INFLATION_STAGE_MAP)
    )
    checks.append(
        _check(
            "Policy governance",
    ...
    checks.append(
        _check(
            "Econometric validity",
            "Unreleased target-month indexes are absent from the live information set",
    ...
    checks.append(
        _check(
            "Operational reliability",
            "Comparable inflation news decompositions reconcile arithmetically",
...
def _write_report(
    ...
    report_dir = settings.project_root / "reports" / "inflation_operational_validation"
    report_dir.mkdir(parents=True, exist_ok=True)
    timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    path = report_dir / f"model1b_live_validation_{timestamp}.md"
    identity = current_inflation_model_identity()
    lines = [
        f"# MacroPulse Model 1B v{identity.model_version} Governed Live Validation",
        "",
...
def run_inflation_operational_validation(
    ...
    run_frame = repository.query_df(
        "SELECT * FROM inflation_live_runs WHERE run_id = ?", [run_id]
    )
    if run_frame.empty:
        raise RuntimeError(f"Governed Model 1B run {run_id} does not exist.")
    ...
    components = repository.query_df(
        "SELECT * FROM inflation_live_components WHERE run_id = ? ORDER BY target_series, model_name",
        [run_id],
    )
    coefficients = repository.query_df(
        "SELECT * FROM inflation_coefficients WHERE run_id = ? ORDER BY target_series, model_name, feature",
        [run_id],
    )
    information_set = repository.inflation_live_information_set(run_id)
    ...
    candidate = repository.query_df(
        "SELECT status, passed_checks, failed_checks, warnings FROM inflation_validation_runs WHERE validation_id = ?",
        [candidate_id],
    )
    candidate_passed = (
        not candidate.empty
        and str(candidate.iloc[0]["status"]) == "pass"
        and int(candidate.iloc[0]["failed_checks"]) == 0
        and int(candidate.iloc[0]["warnings"]) == 0
    )
    candidate_check = pd.DataFrame(
        [
            _check(
                ...
                (
                    f"{candidate.iloc[0]['status']} / {int(candidate.iloc[0]['passed_checks'])} passed"
        ...
        [
            {
                ...
                "notes": (
                    f"Model 1B v{identity.model_version} "
                    f"{identity.lifecycle_status} governed live operational validation."
                ),
            }
        ]
    )
    repository.save_inflation_validation_outputs(run_record, persisted)
    return {
        "validation_id": validation_id,
        "run_id": run_id,
        "status": status,
