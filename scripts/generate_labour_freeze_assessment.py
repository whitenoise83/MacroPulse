...
def _latest_validation(
    ...
    if requested_id:
        work = work.loc[
            (work["validation_id"].astype(str) == requested_id)
            & (work["_type"] == validation_type)
        ]
    else:
...
def assess_freeze_readiness(
    ...
    validations = repository.query_df(
        "SELECT * FROM labour_validation_runs ORDER BY created_at DESC"
    )
    ...
    checks.append(
        _check(
            "Candidate evidence",
            "Passing Model 1C candidate validation is available",
            candidate_exists
            and str(candidate["status"]) == "pass"
            and int(candidate["passed_gates"]) == EXPECTED_CANDIDATE_PASSES
            and int(candidate["failed_gates"]) == 0
            and int(candidate["warning_gates"]) == 0,
            (
                f"{candidate['status']} / {int(candidate['passed_gates'])} passed / "
                f"{int(candidate['failed_gates'])} failed / "
                f"{int(candidate['warning_gates'])} warnings"
                if candidate_exists
                else "missing"
            ),
    ...
    checks.append(
        _check(
            ...
            and int(operational["passed_gates"]) == EXPECTED_OPERATIONAL_PASSES
            and int(operational["failed_gates"]) == 0
            and int(operational["warning_gates"]) == 0,
            (
                f"{operational['status']} / {int(operational['passed_gates'])} passed / "
                f"{int(operational['failed_gates'])} failed / "
                f"{int(operational['warning_gates'])} warnings"
    ...
    live_runs = (
        repository.query_df(
            "SELECT * FROM labour_live_runs WHERE run_id = ?", [live_run_id]
        )
        if live_run_id
        else pd.DataFrame()
    )
    ...
    target_count = (
        int(forecasts["target_series"].nunique()) if not forecasts.empty else 0
    )
    duplicate_count = (
        int(forecasts.duplicated(["target_series"]).sum())
    ...
    checks.append(
        _check(
            "Live governance",
            "Governed run contains exactly one headline for each labour target",
    ...
    for row in forecasts.itertuples(index=False):
        expected = STABLE_POLICY.get(str(row.target_series), {}).get(
    ...
    successful_news = (
        news.loc[news["status"].astype(str) == "success"].copy()
        if not news.empty
        else pd.DataFrame()
    )
    news_targets = (
        int(successful_news["target_series"].nunique())
        if not successful_news.empty
        else 0
    )
    residual = (
        float(
            pd.to_numeric(
                successful_news["residual_interaction"], errors="coerce"
            )
            .abs()
            .max()
        )
    ...
    previous_ids = (
        sorted(set(successful_news["previous_run_id"].dropna().astype(str)))
        if not successful_news.empty
        else []
    )
    checks.append(
        _check(
            "News decomposition",
            "Comparable news decomposition exists for all three governed headlines",
    ...
    checks.append(
        _check(
            "News decomposition",
            "Labour news decomposition reconciles arithmetically",
    ...
    if live_exists:
        invalid_hashes = [
            ...
            if not isinstance(live.get(field), str)
            or len(str(live.get(field))) != 64
        ]
    else:
    ...
    candidate_report_ok = (
        candidate_exists and _report_exists(candidate["report_path"])
    )
    operational_report_ok = (
        operational_exists and _report_exists(operational["report_path"])
    )
    checks.append(
        _check(
            "Documentation",
    ...
    model_card = settings.project_root / "docs" / "model1c_model_card_draft.md"
    checks.append(
        _check(
            "Documentation",
            "Model 1C model card is present",
            model_card.is_file(),
            str(model_card),
            "file exists",
        )
    ...
    return {
        ...
        "operational_validation_id": (
            str(operational["validation_id"]) if operational_exists else ""
        ),
        "live_run_id": live_run_id,
...
def write_report(assessment_id: str, result: dict[str, Any]) -> Path:
    report_dir = settings.project_root / "reports" / "labour_freeze"
    report_dir.mkdir(parents=True, exist_ok=True)
    timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    path = report_dir / f"model1c_freeze_assessment_{timestamp}.md"
    lines = [
        ...
        f"- Stable policy decisions: "
        f"`{sum(len(value) for value in STABLE_POLICY.values())}`",
    ...
    if result["readiness"] == "ready_for_model_owner_signoff":
        lines.append(
            "The Model 1C candidate has passing pseudo-real-time candidate "
            "evidence, a passing governed live operational validation, complete "
            "provenance hashes, a comparable three-target news decomposition, "
            "and preserved validation reports. It is ready for explicit "
            "model-owner review and signoff. This assessment does not itself "
            "promote the model to production."
        )
    else:
        lines.append(
            "The Model 1C candidate is not ready for owner signoff. Resolve the "
            "failed freeze checks and regenerate this assessment."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Assess Model 1C governed candidate readiness for model-owner signoff."
        )
    )
        ...
        [
            {
                ...
                "warning_gates": result["warnings"],
                "report_path": str(report_path),
                "summary_json": json.dumps(
                    {
                        "validation_type": "freeze_assessment",
                        "readiness": result["readiness"],
                        "candidate_validation_id": result[
                            "candidate_validation_id"
                        ],
                        "operational_validation_id": result[
                            "operational_validation_id"
                        ],
                ...
                "notes": (
                    "Model 1C governed-candidate freeze assessment; "
    ...
    print("Model 1C freeze assessment complete")
